                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                      <section class="text-gray-600 body-font">
                        <div class="container px-5 py-8 mx-auto">
                          ...
                          <div class="flex pl-4 my-4 lg:w-2/3 w-full mx-auto">
                            <div>
                              ...
                              <button class="bg-blue-300 text-white ml-1 py-2 px-2" @click="searchCustomers">検索</button>
                            </div>
                          </div>
                              ...
                              <tbody>
                                ...
                                <tr v-for="order in props.orders.data" :key="order.id">
                                  <td class="border-b-2 border-gray-200 px-4 py-3">
                                    <Link class="text-blue-400" :href="route('purchases.show', { purchase: order.id })"> <!-- { purchase:はphp artisan route:listから確認 -->
                                      {{ order.id }}
                                    </Link>
                                  </td>
                                  <td class="border-b-2 border-gray-200 px-4 py-3">{{ order.customer_name }}</td>
                                  <td class="border-b-2 border-gray-200 px-4 py-3">{{ Number(order.total).toLocaleString() }}</td>
                        ...
                        <Pagination class="flex justify-center mt-6" :links="props.orders.links"></Pagination>
                      </section>
                    </div>
                </div>
            </div>

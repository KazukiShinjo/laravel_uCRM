...
import BreezeVaildationErrors from '@/Components/VaildationErrors.vue';
...
// onMountedは、画面に表示されたタイミングで一度だけ実行される処理
onMounted(() => {
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                  <BreezeVaildationErrors class="mb-4" />
                        ...
                        <div class="container px-5 py-8 mx-auto">
                          <div class="lg:w-1/2 md:w-2/3 mx-auto">
                            <div class="flex flex-wrap -m-2">
                                ...
                                <table class="table-auto w-full text-left whitespace-no-wrap">
                                  ...
                                  <tbody>
                                    ...
                                    <tr v-for="item in itemList" :key="item.id">
                                      ...
                                      <td class="border-b-2 border-gray-200 px-4 py-3">{{ Number(item.price * item.quantity).toLocaleString() }}</td>
                                    </tr>
                                  </tbody>
                                </table>
                              </div>
                              ...
                              <div class="p-2 w-full">
                                <div class="relative">
                                  <label for="status" class="leading-7 ml-2 mr-4 text-sm text-gray-600">ステータス</label>
                                  <input type="radio" id="status" v-model="form.status" name="status" :value="1">
                                  <label for="is_selling" class="ml-2 mr-4 text-sm text-gray-600">未キャンセル</label>
                                  <input type="radio" id="status" v-model="form.status" name="status" :value="0">
                                  <label for="is_selling" class="ml-2 mr-4 text-sm text-gray-600">キャンセルする</label>
                                </div>
                              </div>
                              <div class="p-2 w-full">

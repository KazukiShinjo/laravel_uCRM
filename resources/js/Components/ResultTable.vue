...
<template>
  <div v-if="data.type === 'decile'" class="lg:w-2/3 w-full mt-3 mx-auto overflow-auto">
    <table class="table-auto w-full text-left whitespace-no-wrap">
      ...
      <tbody>
        ...
        <tr v-for="item in data.data" :key="item.date">
          ...
          <td class="border-b-2 border-gray-200 px-4 py-3">{{ Number(item.average).toLocaleString() }}</td>
          <td class="border-b-2 border-gray-200 px-4 py-3">{{ Number(item.totalPerGroup).toLocaleString() }}</td>
          <td class="border-b-2 border-gray-200 px-4 py-3">{{ item.totalRatio }}%</td>
        </tr>
      </tbody>
    </table>
  </div>
  <div v-if="data.type === 'perDay' || data.type === 'perMonth' || data.type === 'perYear'" class="lg:w-2/3 w-full mt-3 mx-auto overflow-auto">
    <table class="table-auto w-full text-left whitespace-no-wrap">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr v-for="item in data.data" :key="item.date">
          ...
          <td class="border-b-2 border-gray-200 px-4 py-3">{{ Number(item.total).toLocaleString() }}</td>
        </tr>
      </tbody>
    </table>
  </div>

  <div v-if="data.type === 'rfm'" class="lg:w-2/3 w-full mt-3 mx-auto overflow-auto">
